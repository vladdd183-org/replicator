...
class RedisStreamsEventBus:
    ...
    _consumer_task_group: anyio.abc.TaskGroup | None = field(default=None, init=False)
    ...
    async def start(self) -> None:
        ...
        # Запускаем consumer в фоне (anyio)
        self._consumer_task_group = anyio.create_task_group()
        await self._consumer_task_group.__aenter__()
        self._consumer_task_group.start_soon(self._consume_loop)
    ...
    async def stop(self) -> None:
        ...
        if self._consumer_task_group:
            self._consumer_task_group.cancel_scope.cancel()
            await self._consumer_task_group.__aexit__(None, None, None)
            self._consumer_task_group = None
    ...
    async def _consume_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                await anyio.sleep(1)  # Backoff при ошибках
