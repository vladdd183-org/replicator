...
class FullTextSearchQuery(Query[FullTextSearchInput, FullTextSearchOutput]):
    ...
    async def execute(self, params: FullTextSearchInput) -> FullTextSearchOutput:
        ...
        if params.entity_types:
            query = query.where(SearchIndex.entity_type.is_in(params.entity_types))
            count_query = count_query.where(SearchIndex.entity_type.is_in(params.entity_types))

        # Simple text search (LIKE)
        # In production, use proper FTS
        search_term = f"%{params.query}%"
        query = query.where(
            (SearchIndex.title.like(search_term)) | (SearchIndex.content.like(search_term))
        )
        ...
        entries = await (
            query.order_by(SearchIndex.boost, ascending=False)
    ...
    async def _calculate_facets(
        ...
        all_entries = await (
            ...
            .where((SearchIndex.title.like(search_term)) | (SearchIndex.content.like(search_term)))
            .limit(1000)
        )
