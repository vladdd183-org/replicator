...
class RefreshTokenAction(Action[RefreshTokenRequest, TokenPair, UserError]):
    ...
    async def run(self, data: RefreshTokenRequest) -> Result[TokenPair, UserError]:
        ...
        if payload is None:
            return Failure(InvalidCredentialsError(message="Invalid or expired refresh token"))

        # Check token type
        if payload.type != "refresh":
            return Failure(
                InvalidCredentialsError(message="Invalid token type - expected refresh token")
            )
        ...
        if user is None:
            return Failure(InvalidCredentialsError(message="User not found"))
