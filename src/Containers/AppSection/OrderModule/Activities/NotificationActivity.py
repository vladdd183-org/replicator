...
async def send_order_confirmation(data: NotificationInput) -> bool:
    ...
    activity.logger.info(
        f"📧 Sending order confirmation to user {data.user_id} for order {data.order_id}"
    )
    ...
    activity.logger.info(f"✅ Notification sent for order {data.order_id}")
...
async def send_order_cancelled(data: NotificationInput) -> bool:
    ...
    """
    activity.logger.info(
        f"📧 Sending cancellation notice to user {data.user_id} for order {data.order_id}"
    )

    # In production: send email and push notification

    activity.logger.info(f"✅ Cancellation notice sent for order {data.order_id}")
