...
with workflow.unsafe.imports_passed_through():
    ...
    from src.Containers.AppSection.OrderModule.Activities.ReserveInventoryActivity import InventoryItemInput
...
class CreateOrderWorkflow:
    ...
    async def run(
        ...
        workflow.logger.info(
            f"🎭 Starting CreateOrderWorkflow for user {data.user_id} with {len(data.items)} items"
        )
        ...
        try:
            ...
            try:
                ...
                workflow.logger.warning(f"⚠️ Notification failed (non-critical): {notify_error}")
            ...
            workflow.logger.info(
                f"🎉 CreateOrderWorkflow completed successfully! Order: {order.order_id}"
            )
            ...
            workflow.logger.error(f"❌ Order creation failed at step '{self._failed_step}': {ex}")

            # Run all compensations in reverse order
            if not comp.is_empty:
                workflow.logger.info(f"🔙 Running {len(comp)} compensations...")
            ...
            return Failure(
                OrderWorkflowError(
                    message=f"Order creation failed: {ex!s}",
